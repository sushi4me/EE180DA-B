...
printf "Choose a spot <1, 17, 25, 35, 41, 47> : "
read POSITION

while [ "$POSITION" -ne 1 ] && [ "$POSITION" -ne 17 ] && [ "$POSITION" -ne 25 ] && [ "$POSITION" -ne 35 ] && [ "$POSITION" -ne 41 ] && [ "$POSITION" -ne 47 ]; do
...
while [ 0 ]; do
	...
	printf "Go to next position and enter next command <q, f, r> : "
	read VAR
